...
for f in nouns.hif shemp.hif extrawords.hif milot.hif biza-nouns.hif
  do
    iconv -f windows-1255 $f | grep -v ',סמיכות' | grep -v ',של/' | egrep '^[א-תךםןףץ]{3}ייה ' | cut -b 1-8,11-12 >> /meduyeket/word_lists/hspell-kitlia.txt
done
